...
| 1.6 | 集成现有 storage 模块（SQLite） | 2h | 1.5 | 待开始 | 持久化功能正常工作 |
| 1.7 | 编写单元测试 | 4h | 1.6 | 待开始 | 测试覆盖率 > 80% |
| 1.8 | 阶段测试和修复 | 2h | 1.7 | 待开始 | 所有测试通过，功能验证 |
...
| ID | 任务 | 工时 | 依赖 | 状态 | 验收标准 |
|----|------|------|------|------|----------|
| 2.1 | 设计 ContextBuilder 结构 | 1h | 1.8 | 待开始 | 设计文档完成 |
| 2.2 | 实现 ContextBuilder 基础结构 | 2h | 2.1 | 待开始 | context.rs 基础实现 |
| 2.3 | 实现最近对话上下文策略 | 2h | 2.2 | 待开始 | 可以获取最近 N 条对话 |
| 2.4 | 实现相关历史上下文策略 | 2h | 2.3 | 待开始 | 可以基于语义搜索相关历史 |
| 2.5 | 实现用户偏好上下文策略 | 1h | 2.4 | 待开始 | 可以获取用户偏好信息 |
| 2.6 | 实现上下文窗口管理 | 2h | 2.5 | 待开始 | 可以限制上下文 token 数量 |
| 2.7 | 编写测试用例 | 2h | 2.6 | 待开始 | 各种上下文策略测试通过 |
| 2.8 | 阶段测试和修复 | 2h | 2.7 | 待开始 | 所有测试通过 |
...
| ID | 任务 | 工时 | 依赖 | 状态 | 验收标准 |
|----|------|------|------|------|----------|
| 3.1 | 在 bot-runtime 创建 memory_middleware.rs | 1h | 1.8, 2.8 | 待开始 | 中间件基础结构创建 |
| 3.2 | 实现 Middleware trait | 2h | 3.1 | 待开始 | MemoryMiddleware 实现 |
| 3.3 | 实现消息保存逻辑 | 2h | 3.2 | 待开始 | 用户消息自动保存 |
| 3.4 | 实现上下文检索逻辑 | 2h | 3.3 | 待开始 | 自动检索相关上下文 |
