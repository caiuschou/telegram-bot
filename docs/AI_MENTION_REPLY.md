# Bot 被@时AI回复技术方案

## 📋 开发实施清单

---

### 阶段一：准备工作

#### 1.1 配置环境变量

创建或更新 `.env` 文件：

```env
# 必需配置
BOT_TOKEN=your_bot_token                    # Telegram Bot Token
OPENAI_API_KEY=your_openai_api_key          # OpenAI API Key

# 可选配置（有默认值）
OPENAI_BASE_URL=https://api.openai.com/v1    # OpenAI API 地址
DATABASE_URL=file:./telegram_bot.db         # 数据库路径
RATE_LIMIT_MAX_REQUESTS=10                  # 速率限制请求数
RATE_LIMIT_WINDOW_SECS=60                   # 速率限制窗口（秒）
RUST_LOG=info                               # 日志级别
```

- [ ] 创建/更新 `.env` 文件
- [ ] 配置 BOT_TOKEN（从 @BotFather 获取）
- [ ] 配置 OPENAI_API_KEY（从 OpenAI 获取）
- [ ] 根据需要调整其他配置

#### 1.2 更新依赖

编辑 `dbot-cli/Cargo.toml`：

```toml
[dependencies]
telegram-bot-ai = { path = "../telegram-bot-ai" }
openai-client = { path = "../openai-client" }
```

- [ ] 修改 `dbot-cli/Cargo.toml`
- [ ] 添加 telegram-bot-ai 依赖
- [ ] 添加 openai-client 依赖

---

### 阶段二：HandlerChain 机制

#### 2.1 创建 HandlerChain

新建 `bot-runtime/src/handler_chain.rs`

- [ ] 创建文件 `bot-runtime/src/handler_chain.rs`
- [ ] 实现 `HandlerChain` 结构体
- [ ] 实现 `add_handler()` 方法
- [ ] 实现 `handle()` 方法（链式处理逻辑）

#### 2.2 导出 HandlerChain

编辑 `bot-runtime/src/lib.rs`：

```rust
pub mod handler_chain;
pub use handler_chain::HandlerChain;
```

- [ ] 修改 `bot-runtime/src/lib.rs`
- [ ] 添加 handler_chain 模块
- [ ] 导出 HandlerChain

#### 2.3 验证 HandlerChain

- [ ] 运行 `cargo check` 确保编译通过
- [ ] 运行 `cargo run` 验证 bot 正常启动

**阶段二完成后**：
- ✅ 可以编译运行
- ✅ bot 正常启动
- ❌ HandlerChain 未实际使用，功能无变化

---

### 阶段三：AI检测和处理

#### 3.1 创建 AIDetectionHandler

新建 `bot-runtime/src/ai_detection_handler.rs`

- [ ] 创建文件 `bot-runtime/src/ai_detection_handler.rs`
- [ ] 定义 `AIQuery` 结构体
- [ ] 实现 `AIDetectionHandler` 结构体
- [ ] 实现 `is_bot_mentioned()` 方法
- [ ] 实现 `extract_question()` 方法
- [ ] 实现 `Handler` trait 的 `handle()` 方法
- [ ] 检测到@提及时发送到查询队列

#### 3.2 创建 AIQueryHandler

新建 `bot-runtime/src/ai_query_handler.rs`

- [ ] 创建文件 `bot-runtime/src/ai_query_handler.rs`
- [ ] 实现 `AIQueryHandler` 结构体
- [ ] 实现 `run()` 方法（异步任务主循环）
- [ ] 实现 `handle_query()` 方法（处理单个查询）
- [ ] 实现 `send_response()` 方法（发送回复）
- [ ] 实现 `log_ai_response()` 方法（记录AI回复）

#### 3.3 导出 AI 模块

编辑 `bot-runtime/src/lib.rs`：

```rust
pub mod ai_detection_handler;
pub mod ai_query_handler;

pub use ai_detection_handler::{AIDetectionHandler, AIQuery};
```

- [ ] 修改 `bot-runtime/src/lib.rs`
- [ ] 添加 ai_detection_handler 模块
- [ ] 添加 ai_query_handler 模块
- [ ] 导出 AIDetectionHandler 和 AIQuery

#### 3.4 集成 AI 功能

编辑 `dbot-cli/src/main.rs`：

- [ ] 创建 AI 查询通道（mpsc::unbounded_channel）
- [ ] 初始化 `TelegramBotAI`（OpenAI客户端）
- [ ] 初始化 `AIDetectionHandler`（检测层）
- [ ] 初始化 `AIQueryHandler`（处理层）
- [ ] 创建 `HandlerChain` 并添加 handler
- [ ] 在 `teloxide::repl` 中使用 HandlerChain 处理消息
- [ ] 在 tokio::spawn 中启动 `AIQueryHandler::run()`
- [ ] 在 tokio::spawn 中启动 `get_me()` 获取 bot username

#### 3.5 验证 AI 功能

- [ ] 运行 `cargo check` 确保编译通过
- [ ] 运行 `cargo run` 启动 bot
- [ ] 在 Telegram 中 @提及 bot 测试 AI 回复
- [ ] 验证数据库中记录了用户消息和 AI 回复

**阶段三完成后**：
- ✅ 可以编译运行
- ✅ bot 正常启动
- ✅ @提及检测和 AI 回复功能完整

---

### 阶段四：流式响应（可选）

> ⚠️ **本阶段为可选功能**，用于提升用户体验，实现 AI 回复的流式输出。

#### 4.1 更新 AIQueryHandler 支持流式响应

编辑 `bot-runtime/src/ai_query_handler.rs`：

- [ ] 添加 `use_streaming` 配置选项（从环境变量读取）
- [ ] 实现流式响应处理方法 `handle_query_stream()`
- [ ] 添加消息 ID 追踪（用于编辑消息）

#### 4.2 实现流式响应逻辑

- [ ] 调用 `TelegramBotAI::get_ai_response_stream()` 方法
- [ ] 首次收到 chunk 时，发送初始消息 "正在思考..."
- [ ] 收到后续 chunk 时，使用 `bot.edit_message_text()` 更新内容
- [ ] 处理完成后，保存完整回复到数据库

#### 4.3 流式响应代码示例

```rust
async fn handle_query_stream(&self, query: AIQuery) -> Result<()> {
    // 1. 发送初始消息
    let message = self.bot
        .send_message(query.chat_id, "正在思考...")
        .await?;

    // 2. 调用流式 AI API
    let mut full_response = String::new();
    self.ai_bot.get_ai_response_stream(&query.question, |chunk| async {
        full_response += &chunk.content;

        // 3. 编辑消息内容
        self.bot
            .edit_message_text(query.chat_id, message.id, &full_response)
            .await?;

        Ok(())
    }).await?;

    // 4. 记录完整回复
    self.log_ai_response(&query, &full_response).await?;
    Ok(())
}
```

- [ ] 将上述代码逻辑集成到 `handle_query()` 方法
- [ ] 根据 `use_streaming` 配置选择处理方式

#### 4.4 更新配置

编辑 `.env` 文件：

```env
# 流式响应配置
AI_USE_STREAMING=true                       # 是否启用流式响应
AI_THINKING_MESSAGE=正在思考...             # 初始提示文本
```

- [ ] 添加 `AI_USE_STREAMING` 环境变量
- [ ] 添加 `AI_THINKING_MESSAGE` 环境变量

#### 4.5 验证流式响应

- [ ] 运行 `cargo check` 确保编译通过
- [ ] 运行 `cargo run` 启动 bot
- [ ] 在 Telegram 中 @提及 bot
- [ ] 观察消息是否实时更新（流式输出）
- [ ] 切换 `AI_USE_STREAMING=false` 验证非流式模式

**阶段四完成后**：
- ✅ AI 回复支持流式输出
- ✅ 用户体验得到提升
- ✅ 可通过配置切换流式/非流式模式

---

### 阶段五：测试和优化

#### 4.1 代码质量检查

- [ ] 运行 `cargo clippy` 检查代码质量
- [ ] 运行 `cargo fmt` 格式化代码

#### 4.2 功能测试

- [ ] 在 Telegram 中 @提及 bot 测试正常回复
- [ ] 测试 @提及但问题为空的情况
- [ ] 测试非@提及的普通消息（应正常记录）
- [ ] 测试速率限制（快速发送 11 条消息）
- [ ] 检查数据库中正确记录用户消息
- [ ] 检查数据库中正确记录 AI 回复

#### 4.3 错误处理测试

- [ ] 测试无效的 OpenAI API key（应返回错误提示）
- [ ] 测试网络错误情况（应返回错误提示）
- [ ] 测试 OpenAI API 超时（应返回错误提示）
- [ ] 测试 bot 未启动时发送消息（应有日志）

#### 4.4 性能测试

- [ ] 测试单用户快速发送 20 条消息
- [ ] 测试多个用户同时 @提及 bot
- [ ] 观察 AI 响应时间（正常应在 1-5s 内）
- [ ] 检查 CPU 和内存使用情况

**阶段四完成后**：
- ✅ 可以编译运行
- ✅ 所有功能测试通过
- ✅ 错误处理验证完毕
- ✅ 性能指标符合预期

---

### 阶段六：速率限制（可选）

> ⚠️ **本阶段为可选功能**，用于控制 OpenAI API 调用频率，避免产生过高费用。

#### 6.1 创建 RateLimiter

新建 `bot-runtime/src/rate_limiter.rs`

- [ ] 创建文件 `bot-runtime/src/rate_limiter.rs`
- [ ] 定义 `RateLimitConfig` 结构体
- [ ] 实现 `RateLimiter` 结构体
- [ ] 实现 `is_rate_limited()` 方法
- [ ] 实现 `record_request()` 方法（记录请求）

#### 6.2 导出 RateLimiter

编辑 `bot-runtime/src/lib.rs`：

```rust
pub mod rate_limiter;
pub use rate_limiter::{RateLimiter, RateLimitConfig};
```

- [ ] 修改 `bot-runtime/src/lib.rs`
- [ ] 添加 rate_limiter 模块
- [ ] 导出 RateLimiter 和 RateLimitConfig

#### 6.3 集成速率限制到 AIQueryHandler

编辑 `bot-runtime/src/ai_query_handler.rs`：

- [ ] 在 AIQueryHandler 中添加 RateLimiter 字段
- [ ] 在 `handle_query()` 开始时调用 `is_rate_limited()`
- [ ] 如果超限，跳过本次 AI 调用
- [ ] 如果未超限，调用 AI 后执行 `record_request()`

#### 6.4 验证速率限制

- [ ] 运行 `cargo check` 确保编译通过
- [ ] 运行 `cargo run` 启动 bot
- [ ] 快速 @提及 bot 11 次
- [ ] 验证第 11 次被拒绝（速率限制生效）
- [ ] 等待 1 分钟后再发送，验证恢复正常

**阶段五完成后**：
- ✅ AI 调用受到速率限制保护
- ✅ 可有效控制 API 成本
- ✅ 防止恶意用户滥用

---

### 📊 进度跟踪

| 阶段 | 任务数 | 完成数 | 进度 | 可运行 |
|------|--------|--------|------|--------|
| 阶段一：准备工作 | 7 | 0 | 0% | ✅ |
| 阶段二：HandlerChain 机制 | 9 | 0 | 0% | ✅ |
| 阶段三：AI检测和处理 | 29 | 0 | 0% | ✅ |
| 阶段四：流式响应（可选） | 14 | 0 | 0% | ✅ |
| 阶段五：测试和优化 | 16 | 0 | 0% | ✅ |
| 阶段六：速率限制（可选） | 17 | 0 | 0% | ✅ |
| **总计** | **92** | **0** | **0%** | |

> ✅ **前三个阶段完成后都可以编译运行，功能逐步增强**
> ⚠️ **阶段四、六为可选功能，用于提升体验和控制成本**

### 🎯 阶段功能说明

| 阶段 | 完成后功能 | 验证方式 |
|------|-----------|---------|
| 阶段一：准备工作 | 环境配置完成 | `cargo run` 启动正常 |
| 阶段二：HandlerChain 机制 | HandlerChain 模块可用 | `cargo check` 编译通过 |
| 阶段三：AI检测和处理 | 完整 AI 功能可用 | @提及 bot 获得智能回复 |
| 阶段四：流式响应（可选） | AI 回复实时更新 | 观察消息逐步生成 |
| 阶段五：测试和优化 | 功能测试通过，性能良好 | 所有测试用例通过 |
| 阶段六：速率限制（可选） | AI 调用频率限制生效 | 快速 @提及 11 次被拒绝 |

---

### 🎯 核心目标

在 Telegram Bot 中实现 @提及触发 AI 智能回复功能，支持：
- ✅ @提及检测
- ✅ AI 智能回复
- ✅ 异步队列处理
- ✅ 流式响应（可选）
- ✅ 速率限制保护
- ✅ 完整消息记录

### 📂 关键文件清单

| 文件 | 类型 | 阶段 | 说明 |
|------|------|------|------|
| `.env` | 新建/修改 | 1,4,6 | 环境变量、流式和速率限制配置 |
| `dbot-cli/Cargo.toml` | 修改 | 1 | 依赖配置 |
| `bot-runtime/src/handler_chain.rs` | 新建 | 2 | Handler链机制 |
| `bot-runtime/src/ai_detection_handler.rs` | 新建 | 3 | AI检测层 |
| `bot-runtime/src/ai_query_handler.rs` | 新建 | 3,4 | AI处理层（支持流式） |
| `bot-runtime/src/rate_limiter.rs` | 新建 | 6 | AI调用速率限制（可选） |
| `bot-runtime/src/lib.rs` | 修改 | 2,3,6 | 模块导出 |
| `dbot-cli/src/main.rs` | 修改 | 3 | 主程序集成 |

### 🔍 功能特性

| 功能 | 状态 |
|------|------|
| @提及检测 | ✅ |
| AI智能回复 | ✅ |
| Handler链机制 | ✅ 真链 |
| 异步处理 | ✅ 异步队列 |
| 流式响应 | ✅ 可选 |
| 速率限制 | ✅ 可选 |
| 完整消息记录 | ✅ 用户+AI |
| 动态获取bot_username | ✅ |
| 可扩展性 | ✅ 高 |

### 📊 架构设计

```
用户消息 → HandlerChain
              ├─ AIDetectionHandler → AI查询队列 ─┐
              │                                    ↓
              └─ MessageHandler (记录)    AIQueryHandler
                                              ├─ 速率限制（可选）
                                              ├─ OpenAI (普通/流式）
                                              ├─ 发送/编辑消息
                                              └─ 记录到数据库
                                              ↑ 异步处理，不阻塞主流程
```

### ⚠️ 注意事项

1. **异步处理**：AI查询在独立任务中处理，不阻塞主消息流程
2. **Token安全**：切勿将敏感信息提交到代码仓库
3. **API成本控制**：建议配置速率限制（阶段六），控制API调用成本
4. **速率限制定位**：速率限制仅针对 AI 调用，不影响普通消息处理
5. **流式响应**：流式响应会通过编辑消息实时更新，避免发送多条消息
6. **可选功能**：前三个阶段即可实现完整功能，流式响应和速率为增强功能

### 🚀 快速开始

```bash
# 1. 配置环境变量
cp .env.example .env
# 编辑 .env 填入 BOT_TOKEN 和 OPENAI_API_KEY

# 2. 运行 bot
cargo run

# 3. 在 Telegram 中 @提及你的 bot 测试

# 4. (可选) 启用流式响应
# 设置 AI_USE_STREAMING=true，AI 回复将实时显示

# 5. (可选) 添加速率限制
# 完成阶段六后，AI调用将受到速率限制保护
```

---

## 概述

在现有 dbot-cli 基础上实现：当用户在 Telegram 中 @ 提及 bot 时，使用 OpenAI 进行智能回复。

## 架构设计

### 总体架构图

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              Telegram 用户                                      │
│                              发送消息 @bot                                      │
└─────────────────────────────────────┬───────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│                            dbot-cli (主程序)                                     │
│                                                                                  │
│  ┌────────────────────────────────────────────────────────────────────────┐   │
│  │                         主消息循环 (teloxide::repl)                       │   │
│  │                                                                           │   │
│  │  ┌────────────────────────────────────────────────────────────────┐    │   │
│  │  │              HandlerChain (处理器链)                        │    │   │
│  │  │                                                            │    │   │
│  │  │  ┌────────────────────────────────────────────────────┐     │    │   │
│  │  │  │  AIDetectionHandler (AI检测层)                       │     │    │   │
│  │  │  │  ├─ 检测 @bot 提及                                   │     │    │   │
│  │  │  │  ├─ 提取问题内容                                    │     │    │   │
│  │  │  │  ├─ 发送到 AI 查询队列 (async)                       │     │    │   │
│  │  │  │  └─ 返回 Stop (停止后续处理)                         │     │    │   │
│  │  │  └────────────────────────────────────────────────────┘     │    │   │
│  │  │           │ (未检测到 @提及)                              │    │   │
│  │  │           │ Continue                                       │    │   │
│  │  │           ▼                                                │    │   │
│  │  │  ┌────────────────────────────────────────────────────┐     │    │   │
│  │  │  │  MessageHandler (消息记录层)                        │     │    │   │
│  │  │  │  └─ 保存用户消息到数据库                            │     │    │   │
│  │  │  │  └─ 返回 Ignore                                     │     │    │   │
│  │  │  └────────────────────────────────────────────────────┘     │    │   │
│  │  └────────────────────────────────────────────────────────────┘   │    │   │
│  └────────────────────────────────────────────────────────────────────┘   │
│                                      │                                       │
│                                      │ 消息处理完成                           │
│                                      │                                       │
│  ┌────────────────────────────────────────────────────────────────┐        │
│  │           AIQueryHandler (异步任务 - 独立 tokio::spawn)          │        │
│  │                                                                   │        │
│  │  ┌────────────────────────────────────────────────────────────┐ │        │
│  │  │  run(): 从队列接收 AI 查询                                │ │        │
│  │  │  │                                                          │ │        │
│  │  │  │  AIQuery { chat_id, user_id, question }                   │ │        │
│  │  │  │           │                                              │ │        │
│  │  │  │           ▼                                              │ │        │
│  │  │  │  ┌────────────────────────────────────────────────┐    │ │        │
│  │  │  │  │  RateLimiter (速率限制)                         │    │ │        │
│  │  │  │  │  ├─ 检查用户 AI 调用频率                         │    │ │        │
│  │  │  │  │  ├─ 超限 → 跳过本次处理                          │    │ │        │
│  │  │  │  │  └─ 未超限 → 继续处理                            │    │ │        │
│  │  │  │  └────────────────────────────────────────────────┘    │ │        │
│  │  │  │           │                                              │ │        │
│  │  │  │           ▼                                              │ │        │
│  │  │  │  ┌────────────────────────────────────────────────┐    │ │        │
│  │  │  │  │  TelegramBotAI (AI 处理模块)                     │    │ │        │
│  │  │  │  │  └─ 调用 OpenAI Streaming API (可选)            │    │ │        │
│  │  │  │  └────────────────────────────────────────────────┘    │ │        │
│  │  │  │           │                                              │ │        │
│  │  │  │           ▼                                              │ │        │
│  │  │  │  ┌────────────────────────────────────────────────┐    │ │        │
│  │  │  │  │  send_message(): 发送初始消息 "正在思考..."     │    │ │        │
│  │  │  │  │  edit_message(): 收到 chunk 时编辑消息内容 (可选)│    │ │        │
│  │  │  │  └────────────────────────────────────────────────┘    │ │        │
│  │  │  │           │                                              │ │        │
│  │  │  │           ▼                                              │ │        │
│  │  │  │  ┌────────────────────────────────────────────────┐    │ │        │
│  │  │  │  │  log_ai_response(): 记录完整 AI 回复到数据库     │    │ │        │
│  │  │  │  └────────────────────────────────────────────────┘    │ │        │
│  │  │  └────────────────────────────────────────────────────┘ │        │
│  └────────────────────────────────────────────────────────────┘        │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌────────────────────────────────────────────────────────────────────────────┐
│                              外部服务                                        │
│                                                                          │
│  ┌──────────────────────┐        ┌──────────────────────────────────┐   │
│  │   Telegram API      │        │      OpenAI API                  │   │
│  │                      │        │                                  │   │
│  │  - 接收用户消息      │        │  - GPT 模型                      │   │
│  │  - 发送 AI 回复      │        │  - AI 对话生成                  │   │
│  └──────────────────────┘        └──────────────────────────────────┘   │
└────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌────────────────────────────────────────────────────────────────────────────┐
│                              数据存储                                        │
│                                                                          │
│  ┌──────────────────────────────────────────────────────────────────┐    │
│  │                     SQLite 数据库                                │    │
│  │                                                                  │    │
│  │  messages 表:                                                     │    │
│  │  ├─ 用户消息 (Incoming)                                          │    │
│  │  └─ AI 回复   (Outgoing)                                          │    │
│  └──────────────────────────────────────────────────────────────────┘    │
└────────────────────────────────────────────────────────────────────────────┘
```

### 数据流架构图

```
同步消息处理流程 (主线程)
═══════════════════════════════════════════════════════════════════════════

用户消息 @bot
    │
    ▼
┌─────────────────────────────────────────────────────────────────────┐ │
│ HandlerChain (顺序执行)                                              │ │
│                                                                      │ │
│  1. AIDetectionHandler                                              │ │
│     ┌──────────────────────────────────────────────────────────┐   │ │
│     │ 检测 @bot                                                   │   │ │
│     │   ├─ 是 → 提取问题 → 发送到队列 → STOP ──────────────────┐ │   │ │
│     │   │                                                     │ │   │ │
│     │   └─ 否 → CONTINUE                                      │ │   │ │
│     └──────────────────────────────────────────────────────────┘ │   │
│                                │                                    │   │
│                                │ (仅未检测到@时)                     │   │
│                                ▼                                    │   │
│                          2. MessageHandler                         │
│                             ┌──────────────────────────────────┐    │   │
│                             │ 保存用户消息到数据库             │    │   │
│                             └──────────────────────────────────┘    │   │
│                                      │ IGNORE                         │
└──────────────────────────────────────┼─────────────────────────────────┘
                                       │
                                       ▼
                                   消息处理完成


异步 AI 处理流程 (独立任务)
═══════════════════════════════════════════════════════════════════════════

AI 查询队列 (mpsc::unbounded_channel)
    │
    │
    ▼
┌────────────────────────────────────────────────────────────────────┐
│ AIQueryHandler::run() (tokio::spawn)                               │
│                                                                     │
│  while let Some(query) = receiver.recv().await {                  │
│                                                                     │
│    ┌──────────────────────────────────────────────────────────┐   │
│    │ 1. 接收查询:                                               │   │
│    │    AIQuery { chat_id, user_id, question }                  │   │
│    └──────────────────────────────────────────────────────────┘   │
│                           │                                          │
│                           ▼                                          │
│    ┌──────────────────────────────────────────────────────────┐   │
│    │ 2. 速率限制检查 (仅 AI 调用)                               │   │
│    │   RateLimiter::check(user_id)                            │   │
│    │   ├─ 超限 → 跳过本次处理                                   │   │
│    │   └─ 未超限 → 继续                                        │   │
│    └──────────────────────────────────────────────────────────┘   │
│                           │                                          │
│                           ▼                                          │
│    ┌──────────────────────────────────────────────────────────┐   │
│    │ 3. 调用 OpenAI API (非流式或流式):                       │   │
│    │    TelegramBotAI::get_ai_response(question)              │   │
│    │    或                                                     │   │
│    │    TelegramBotAI::get_ai_response_stream(question, cb)    │   │
│    └──────────────────────────────────────────────────────────┘   │
│                           │                                          │
│                           ▼                                          │
│    ┌──────────────────────────────────────────────────────────┐   │
│    │ 4a. 非流式: 发送完整回复                                  │   │
│    │    bot.send_message(chat_id, response)                  │   │
│    │                                                           │   │
│    │ 4b. 流式 (可选): 实时更新消息                             │   │
│    │    ├─ 发送初始消息: "正在思考..."                        │   │
│    │    ├─ 收到 chunk → bot.edit_message(chat_id, msg_id, text)│   │
│    │    └─ 完成 → 完整内容已显示                               │   │
│    └──────────────────────────────────────────────────────────┘   │
│                           │                                          │
│                           ▼                                          │
│    ┌──────────────────────────────────────────────────────────┐   │
│    │ 5. 记录到数据库:                                          │   │
│    │    log_ai_response(response)                             │   │
│    └──────────────────────────────────────────────────────────┘   │
│  }                                                                │
└────────────────────────────────────────────────────────────────────┘
```

### 组件交互时序图

```
用户      Telegram      bot      HandlerChain    队列    AIHandler    RateLimiter  OpenAI    数据库
│            │            │             │            │           │         │          │          │
├─消息─@bot─→│            │             │            │           │         │          │          │
│            ├─接收─────→│             │            │           │         │          │          │
│            │            ├─handle()─────────────────→│           │         │          │          │
│            │            │             │            ├─@提及?──→│         │          │          │
│            │            │             │            │←─Stop────┤         │          │          │
│            │            │             │            │           │         │          │          │
│            │            │             │            ├─send()───────────→│         │          │          │
│            │            │             │            │           │         │          │          │
│            │            │             │            │←─Continue┤         │          │          │
│            │            │             │            │           │         │          │          │
│            │            │             │            ├─save────────────────────────────────→│
│            │            │             │            │           │         │          │          │
│            │            │             │            │           │◄─recv()─┤          │          │
│            │            │             │            │           │         │          │          │
│            │            │             │            │           │         ├─check()─→│         │          │
│            │            │             │            │           │         │←─OK─────┤         │          │
│            │            │             │            │           │         │          │          │
│            │            │             │            │           │         ├─API请求───────────→│          │
│            │            │             │            │           │         │         │←─stream→│          │
│            │            │             │            │           │         │         │←─chunk─→│          │
│            │            │             │            │           │         │         │←─AI回复─┤          │
│            │            │             │            │           │         │         │          │          │
│            │            │             │            │           │         ├─send初始"正在..."──→│          │
│            │            │             │            │           │         ├─edit──chunk───────→│          │
│            │            │             │            │           │         ├─edit──chunk───────→│          │
│            │            │             │            │           │         │         │          │          │
│            │            │             │            │           │         ├─save完整回复────────→│          │
│            │            │             │            │           │         │         │          │          │
│←────────────回复─────────────────────────────────────────────────────────────────────────────────│
│            │            │             │            │           │         │         │          │          │
```

### 系统特性说明

#### 同步 vs 异步处理

| 处理类型 | 组件 | 特点 | 耗时 |
|---------|------|------|------|
| **同步** | AIDetectionHandler | 字符串匹配，极快 | < 1ms |
| **同步** | MessageHandler | 数据库写入，较快 | 1-10ms |
| **异步** | AIQueryHandler (非流式) | AI API 调用，慢 | 1-5s |
| **异步** | AIQueryHandler (流式) | AI Streaming API | 实时更新 |
| **异步** | RateLimiter (可选) | 内存检查，极快 | < 1ms |

#### 队列机制

```
查询队列 (mpsc::unbounded_channel)
    ├─ 类型: 无界通道 (unbounded)
    ├─ 优点: 不会阻塞发送者
    ├─ 缺点: 内存可能无限增长
    └─ 改进: 可考虑有界通道 + 背压机制
```

#### 关键设计决策

1. **为什么异步处理 AI?**
   - AI API 调用耗时 1-5 秒
   - 防止阻塞主消息循环
   - 提升系统吞吐量

2. **为什么分离检测和处理?**
   - 职责单一，易于测试
   - 检测层可快速返回
   - 处理层可独立扩展

3. **为什么使用 HandlerChain?**
    - 可灵活添加新 Handler
    - HandlerResponse 控制流程
    - 便于维护和扩展

4. **为什么支持流式响应?**
    - 提升用户体验，实时看到 AI 生成过程
    - 减少等待时间，降低焦虑感
    - 更符合对话的自然交互体验

5. **流式响应实现方式**
    - 发送初始消息："正在思考..."
    - 收到 chunk 后使用 `editMessageText` 更新内容
    - 避免发送多条消息，保持聊天界面整洁

## 核心实现

### 方案概述

**适用场景**：生产环境、高并发、需要完整功能

### 核心改进

1. 真正的Handler链机制
2. 异步队列处理AI查询
3. 完整的消息记录
4. 支持 AI 流式响应（可选）
5. 可选的 AI 调用速率限制（可选阶段）

#### 1. Handler链机制实现

**新建**: `bot-runtime/src/handler_chain.rs`

```rust
use dbot_core::{Handler, HandlerResponse, Message, Result};
use std::sync::Arc;
use tracing::{info, debug, instrument};

#[derive(Clone)]
pub struct HandlerChain {
    handlers: Vec<Arc<dyn Handler>>,
}

impl HandlerChain {
    pub fn new() -> Self {
        Self { handlers: Vec::new() }
    }

    pub fn add_handler(mut self, handler: Arc<dyn Handler>) -> Self {
        self.handlers.push(handler);
        self
    }

    #[instrument(skip(self, message))]
    pub async fn handle(&self, message: &Message) -> Result<HandlerResponse> {
        let mut final_response = HandlerResponse::Continue;

        for handler in &self.handlers {
            let response = handler.handle(message).await?;
            debug!(
                handler = std::any::type_name_of_val(handler.as_ref()),
                response = ?response,
                "Handler processed"
            );

            match response {
                HandlerResponse::Stop => {
                    info!("Handler chain stopped");
                    final_response = response;
                    break;
                }
                HandlerResponse::Continue => {
                    continue;
                }
                HandlerResponse::Ignore => {
                    continue;
                }
            }
        }

        Ok(final_response)
    }
}
```

#### 2. AI Detection Handler（检测层）

**新建**: `bot-runtime/src/ai_detection_handler.rs`

```rust
use async_trait::async_trait;
use dbot_core::{Handler, HandlerResponse, Message, Result};
use std::sync::Arc;
use tracing::{info, debug, instrument};

#[derive(Clone)]
pub struct AIDetectionHandler {
    bot_username: Arc<tokio::sync::RwLock<Option<String>>>,
    query_sender: Arc<tokio::sync::mpsc::UnboundedSender<AIQuery>>,
}

#[derive(Debug, Clone)]
pub struct AIQuery {
    pub chat_id: i64,
    pub user_id: i64,
    pub question: String,
}

impl AIDetectionHandler {
    pub fn new(
        bot_username: Arc<tokio::sync::RwLock<Option<String>>>,
        query_sender: Arc<tokio::sync::mpsc::UnboundedSender<AIQuery>>,
    ) -> Self {
        Self {
            bot_username,
            query_sender,
        }
    }

    async fn get_bot_username(&self) -> Option<String> {
        self.bot_username.read().await.clone()
    }

    async fn set_bot_username(&self, username: String) {
        *self.bot_username.write().await = Some(username);
    }

    fn is_bot_mentioned(&self, text: &str, bot_username: &str) -> bool {
        text.contains(&format!("@{}", bot_username))
    }

    fn extract_question(&self, text: &str, bot_username: &str) -> String {
        text.replace(&format!("@{}", bot_username), "")
            .trim()
            .to_string()
    }
}

#[async_trait]
impl Handler for AIDetectionHandler {
    #[instrument(skip(self, message))]
    async fn handle(&self, message: &Message) -> Result<HandlerResponse> {
        if let Some(bot_username) = self.get_bot_username().await {
            if self.is_bot_mentioned(&message.content, &bot_username) {
                let question = self.extract_question(&message.content, &bot_username);

                if !question.is_empty() {
                    info!(
                        user_id = message.user.id,
                        question = %question,
                        "Bot mentioned, sending to AI queue"
                    );

                    let query = AIQuery {
                        chat_id: message.chat.id,
                        user_id: message.user.id,
                        question,
                    };

                    self.query_sender
                        .send(query)
                        .map_err(|e| dbot_core::DbotError::Bot(format!("Failed to send AI query: {}", e)))?;

                    return Ok(HandlerResponse::Stop);
                }
            }
        }

        Ok(HandlerResponse::Continue)
    }
}
```

#### 3. AI Query Handler（AI处理层）

**新建**: `bot-runtime/src/ai_query_handler.rs`

```rust
use dbot_core::Message;
use telegram_bot_ai::TelegramBotAI;
use openai_client::OpenAIClient;
use storage::MessageRepository;
use tokio::sync::mpsc::UnboundedReceiver;
use tracing::{info, error, debug};
use teloxide::Bot;
use std::sync::Arc;

pub struct AIQueryHandler {
    ai_bot: TelegramBotAI,
    bot: Arc<Bot>,
    repo: MessageRepository,
    receiver: UnboundedReceiver<AIDetectionHandler::AIQuery>,
}

impl AIQueryHandler {
    pub fn new(
        ai_bot: TelegramBotAI,
        bot: Bot,
        repo: MessageRepository,
        receiver: UnboundedReceiver<AIDetectionHandler::AIQuery>,
    ) -> Self {
        Self {
            ai_bot,
            bot: Arc::new(bot),
            repo,
            receiver,
        }
    }

    pub async fn run(&mut self) {
        while let Some(query) = self.receiver.recv().await {
            self.handle_query(query).await;
        }
    }

    async fn handle_query(&self, query: AIDetectionHandler::AIQuery) {
        info!(
            user_id = query.user_id,
            question = %query.question,
            "Processing AI query"
        );

        match self.ai_bot.get_ai_response(&query.question).await {
            Ok(response) => {
                if let Err(e) = self.send_response(&query, &response).await {
                    error!(error = %e, "Failed to send AI response");
                }
            }
            Err(e) => {
                error!(error = %e, "Failed to get AI response");
                let error_msg = "抱歉，处理您的请求时出错，请稍后重试。";
                let _ = self.send_response(&query, error_msg).await;
            }
        }
    }

    async fn send_response(&self, query: &AIDetectionHandler::AIQuery, response: &str) -> Result<()> {
        let chat_id = teloxide::types::ChatId(query.chat_id);

        self.bot
            .send_message(chat_id, response)
            .await
            .map_err(|e| {
                error!(error = %e, "Failed to send message");
                dbot_core::DbotError::Bot(e.to_string())
            })?;

        self.log_ai_response(query, response).await?;

        info!(user_id = query.user_id, "AI response sent");
        Ok(())
    }

    async fn log_ai_response(&self, query: &AIDetectionHandler::AIQuery, response: &str) -> Result<()> {
        let record = storage::MessageRecord::new(
            query.user_id,
            query.chat_id,
            None,
            None,
            None,
            "ai_response".to_string(),
            response.to_string(),
            "sent".to_string(),
        );

        self.repo
            .save(&record)
            .await
            .map_err(|e| {
                error!(error = %e, "Failed to save AI response");
                dbot_core::DbotError::Database(e.to_string())
            })?;

        Ok(())
    }
}
```

#### 3. Rate Limit Middleware

**新建**: `bot-runtime/src/rate_limit.rs`

```rust
use async_trait::async_trait;
use dbot_core::{Middleware, Message, Result};
use std::collections::HashMap;
use std::time::{Duration, Instant};
use tokio::sync::Mutex;
use tracing::{warn, info};

#[derive(Clone)]
pub struct RateLimitConfig {
    pub max_requests: usize,
    pub window_duration: Duration,
}

impl Default for RateLimitConfig {
    fn default() -> Self {
        Self {
            max_requests: 10,
            window_duration: Duration::from_secs(60),
        }
    }
}

#[derive(Clone)]
pub struct RateLimitMiddleware {
    config: RateLimitConfig,
    requests: Arc<Mutex<HashMap<i64, Vec<Instant>>>>,
}

impl RateLimitMiddleware {
    pub fn new(config: RateLimitConfig) -> Self {
        Self {
            config,
            requests: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    async fn is_rate_limited(&self, user_id: i64) -> bool {
        let now = Instant::now();
        let mut requests = self.requests.lock().await;

        let user_requests = requests.entry(user_id).or_insert_with(Vec::new);

        user_requests.retain(|&t| now.duration_since(t) < self.config.window_duration);

        if user_requests.len() >= self.config.max_requests {
            warn!(
                user_id = user_id,
                requests = user_requests.len(),
                "User rate limited"
            );
            true
        } else {
            user_requests.push(now);
            false
        }
    }
}

#[async_trait]
impl Middleware for RateLimitMiddleware {
    #[instrument(skip(self, message))]
    async fn before(&self, message: &Message) -> Result<bool> {
        if self.is_rate_limited(message.user.id).await {
            info!(user_id = message.user.id, "Message blocked by rate limit");
            Ok(false)
        } else {
            Ok(true)
        }
    }

    async fn after(&self, _message: &Message, _response: &HandlerResponse) -> Result<()> {
        Ok(())
    }
}
```

#### 4. 更新 bot-runtime/src/lib.rs

```rust
pub mod handler;
pub mod state;
pub mod middleware;
pub mod handler_chain;
pub mod ai_detection_handler;
pub mod ai_query_handler;
pub mod rate_limit;

pub use handler::MessageHandler;
pub use state::StateManager;
pub use handler_chain::HandlerChain;
pub use ai_detection_handler::{AIDetectionHandler, AIQuery};
pub use rate_limit::{RateLimitMiddleware, RateLimitConfig};
```

#### 6. 更新 dbot-cli/src/main.rs

```rust
use anyhow::Result;
use clap::Parser;
use teloxide::prelude::*;
use dbot_core::{Chat, Message, User, MessageDirection, ToCoreMessage, ToCoreUser, Handler, init_tracing};
use bot_runtime::{MessageHandler, StateManager, HandlerChain, AIDetectionHandler, RateLimitMiddleware, RateLimitConfig};
use storage::MessageRepository;
use std::sync::Arc;
use telegram_bot_ai::TelegramBotAI;
use openai_client::OpenAIClient;
use tracing::{info, instrument, error};

#[derive(Parser)]
#[command(name = "dbot")]
#[command(about = "运行 Telegram Bot", long_about = None)]
#[command(version)]
struct Cli {
    #[arg(short, long)]
    token: Option<String>,
}

#[tokio::main]
async fn main() -> Result<()> {
    dotenvy::dotenv().ok();
    let cli = Cli::parse();
    run_bot(cli.token).await
}

#[instrument(skip(token))]
async fn run_bot(token: Option<String>) -> Result<()> {
    let log_file = "logs/telegram-bot.log";
    std::fs::create_dir_all("logs").expect("Failed to create logs directory");
    init_tracing(log_file)?;

    let bot_token = token.unwrap_or_else(|| {
        std::env::var("BOT_TOKEN").expect("BOT_TOKEN not set")
    });

    let database_url =
        std::env::var("DATABASE_URL").unwrap_or_else(|_| "file:./telegram_bot.db".to_string());

    let openai_api_key = std::env::var("OPENAI_API_KEY").expect("OPENAI_API_KEY not set");
    let openai_base_url = std::env::var("OPENAI_BASE_URL")
        .unwrap_or_else(|_| "https://api.openai.com/v1".to_string());

    let rate_limit_max: usize = std::env::var("RATE_LIMIT_MAX_REQUESTS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(10);
    let rate_limit_window_secs: u64 = std::env::var("RATE_LIMIT_WINDOW_SECS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(60);

    info!(database_url = %database_url, "Initializing bot");

    let repo = Arc::new(
        MessageRepository::new(&database_url)
            .await
            .map_err(|e| {
                error!(error = %e, database_url = %database_url, "Failed to initialize message storage");
                e
            })
            .expect("Failed to initialize message storage"),
    );

    let teloxide_bot = Bot::new(bot_token.clone());

    let bot_username = Arc::new(tokio::sync::RwLock::new(None));

    let (query_sender, query_receiver) = tokio::sync::mpsc::unbounded_channel();

    let openai_client = OpenAIClient::with_base_url(openai_api_key, openai_base_url);
    let ai_bot = TelegramBotAI::new("bot".to_string(), openai_client);

    let mut ai_query_handler = bot_runtime::AIQueryHandler::new(
        ai_bot,
        teloxide_bot.clone(),
        repo.as_ref().clone(),
        query_receiver,
    );

    tokio::spawn(async move {
        ai_query_handler.run().await;
    });

    let ai_detection_handler = Arc::new(AIDetectionHandler::new(
        bot_username.clone(),
        Arc::new(query_sender),
    ));

    let rate_limit_middleware = Arc::new(RateLimitMiddleware::new(RateLimitConfig {
        max_requests: rate_limit_max,
        window_duration: Duration::from_secs(rate_limit_window_secs),
    }));

    let message_handler = Arc::new(MessageHandler::new(repo.as_ref().clone()));

    let handler_chain = HandlerChain::new()
        .add_handler(ai_detection_handler)
        .add_handler(message_handler);

    let state_manager = StateManager::new();

    info!("Bot started successfully");

    let mut bot_username_ref = bot_username.clone();
    tokio::spawn(async move {
        if let Ok(me) = teloxide_bot.get_me().await {
            *bot_username_ref.write().await = Some(me.user.username);
            info!(username = %me.user.username, "Bot username set");
        }
    });

    teloxide::repl(teloxide_bot, move |_bot: Bot, msg: teloxide::types::Message| {
        let handler_chain = handler_chain.clone();
        let rate_limit_middleware = rate_limit_middleware.clone();

        async move {
            if let Some(text) = msg.text() {
                let wrapper = TelegramMessageWrapper(&msg);
                let core_msg = wrapper.to_core();

                info!(
                    user_id = core_msg.user.id,
                    message_content = %text,
                    "Received message"
                );

                if let Err(e) = rate_limit_middleware.before(&core_msg).await {
                    error!(error = %e, user_id = core_msg.user.id, "Rate limit check failed");
                    return Ok(());
                }

                match handler_chain.handle(&core_msg).await {
                    Ok(_) => {}
                    Err(e) => {
                        error!(error = %e, user_id = core_msg.user.id, "Handler chain failed");
                    }
                }

                let _ = rate_limit_middleware.after(&core_msg, &dbot_core::HandlerResponse::Ignore).await;
            }
            Ok(())
        }
    })
    .await;

    Ok(())
}

struct TelegramUserWrapper<'a>(&'a teloxide::types::User);

impl<'a> ToCoreUser for TelegramUserWrapper<'a> {
    fn to_core(&self) -> User {
        User {
            id: self.0.id.0 as i64,
            username: self.0.username.clone(),
            first_name: Some(self.0.first_name.clone()),
            last_name: self.0.last_name.clone(),
        }
    }
}

struct TelegramMessageWrapper<'a>(&'a teloxide::types::Message);

impl<'a> ToCoreMessage for TelegramMessageWrapper<'a> {
    fn to_core(&self) -> Message {
        Message {
            id: self.0.id.to_string(),
            user: self.0.from.as_ref()
                .map(|u| TelegramUserWrapper(u).to_core())
                .unwrap_or_else(|| User {
                    id: 0,
                    username: None,
                    first_name: None,
                    last_name: None,
                }),
            chat: Chat {
                id: self.0.chat.id.0,
                chat_type: format!("{:?}", self.0.chat.kind),
            },
            content: self.0.text().unwrap_or("").to_string(),
            message_type: "text".to_string(),
            direction: MessageDirection::Incoming,
            created_at: chrono::Utc::now(),
        }
    }
}
```

#### 7. 更新 dbot-cli/Cargo.toml

```toml
[dependencies]
dbot-core = { path = "../dbot-core" }
bot-runtime = { path = "../bot-runtime" }
storage = { path = "../storage" }
telegram-bot-ai = { path = "../telegram-bot-ai" }
openai-client = { path = "../openai-client" }
teloxide = { version = "0.12", features = ["macros"] }
anyhow = "1.0"
clap = { version = "4.4", features = ["derive"] }
tokio = { version = "1", features = ["full"] }
tracing = "0.1"
tracing-subscriber = { version = "0.3", features = ["env-filter", "json"] }
dotenvy = "0.15"
chrono = "0.4"
```

## 配置要求

```env
# Telegram Bot 配置
BOT_TOKEN=your_bot_token

# OpenAI 配置
OPENAI_API_KEY=your_openai_api_key
OPENAI_BASE_URL=https://api.openai.com/v1

# 数据库配置
DATABASE_URL=file:./telegram_bot.db

# 流式响应配置（可选）
AI_USE_STREAMING=true                       # 是否启用流式响应
AI_THINKING_MESSAGE=正在思考...             # 初始提示文本

# 速率限制配置（可选）
RATE_LIMIT_MAX_REQUESTS=10                  # 速率限制请求数
RATE_LIMIT_WINDOW_SECS=60                   # 速率限制窗口（秒）

# 日志级别
RUST_LOG=info
```

## 数据流

### 数据流

```
1. 用户发送消息到 Telegram
   ↓
2. teloxide 接收消息
   ↓
3. 转换为 Message 对象
   ↓
4. HandlerChain 依次处理
   ├─ AIDetectionHandler
   │   ├─ 检测是否 @ 提及 bot
   │   ├─ 是：发送到 AI 查询队列，返回 Stop
   │   └─ 否：返回 Continue
   ├─ MessageHandler
   │   ├─ 保存消息到数据库
   │   └─ 返回 Ignore
   ↓
5. AIQueryHandler（异步任务）
   ├─ 从队列接收查询
   ├─ 速率限制检查（可选）
   ├─ 调用 OpenAI API（普通或流式）
   ├─ 发送/编辑 AI 回复
   └─ 记录到数据库
```

## Handler 响应说明

- `HandlerResponse::Stop`: 停止后续 Handler 处理
- `HandlerResponse::Continue`: 继续后续 Handler 处理
- `HandlerResponse::Ignore`: 消息已处理完成，不影响流程

## 架构优势

1. **关注点分离**
   - 检测、处理、发送、记录各司其职
   - 易于测试和维护

2. **异步处理**
   - AI查询在独立任务中处理
   - 不阻塞主消息处理流程

3. **完整的Handler链**
   - HandlerResponse真正起作用
   - 可以灵活控制处理流程

4. **速率限制保护**
    - 防止API滥用
    - 保护系统稳定性

5. **流式响应体验**
    - 实时显示 AI 生成过程
    - 减少用户等待焦虑
    - 提升交互自然度

6. **流式响应体验**
    - 实时显示 AI 生成过程
    - 减少用户等待焦虑
    - 提升交互自然度

7. **完整的消息记录**
   - 记录用户消息和AI回复
   - 便于审计和分析

6. **动态配置**
    - bot_username从API获取
    - 速率限制可配置
    - 流式响应可开关

7. **模块化设计**
   - 复用现有模块
   - 不影响现有功能

8. **类型安全**
   - 基于Rust类型系统
   - 完整的错误处理

## 测试建议

### 单元测试
1. HandlerChain的执行顺序
2. 流式响应逻辑
3. 速率限制逻辑
4. @提及检测
5. 问题提取

### 集成测试
1. 完整的AI回复流程（非流式）
2. 完整的AI回复流程（流式）
3. 异步队列处理
4. 数据库记录
5. 错误处理
6. 流式消息编辑

### 性能测试
1. 高并发场景
2. 流式响应性能
3. 速率限制生效性
4. 队列堆积处理
5. 响应时间（普通 vs 流式）

### 手动测试
1. 在Telegram中@提及bot
2. 验证AI回复是否正确
3. 验证流式响应是否正常（可选）
4. 验证消息是否被记录到数据库
5. 测试速率限制（可选）
6. 测试流式响应切换（可选）

## 后续优化方向

1. **对话上下文**
    - 从数据库获取历史消息
    - 支持多轮对话
    - 添加记忆管理

2. **消息发送队列**
    - 处理发送失败
    - 自动重试机制
    - 指数退避策略
    - 支持优先级队列

3. **缓存机制**
    - 相似问题缓存
    - 减少 API 调用
    - 降低成本
    - 支持 Redis 集成

4. **监控和告警**
    - API 使用量监控
    - 异常情况告警
    - 性能指标收集
    - Prometheus 集成

5. **用户权限管理**
    - 白名单/黑名单
    - 基于角色的访问控制
    - 支持群组管理

6. **多模型支持**
    - 支持切换不同 AI 模型
    - 根据问题复杂度自动选择
    - 支持 GPT-4、Claude 等

7. **Markdown 支持**
    - 支持 Markdown 格式渲染
    - 代码块语法高亮
    - 支持图片、链接等

8. **有界队列优化**
    - 从 unbounded 改为有界通道
    - 实现背压机制
    - 防止内存溢出
   - 减少API调用
   - 降低成本

5. **监控和告警**
   - API使用量监控
   - 异常情况告警
   - 性能指标收集

6. **命令前缀支持**
   - 添加 `/ai` 或 `/ask` 命令
   - 私聊模式自动回复

7. **用户权限管理**
   - 白名单/黑名单
   - 基于角色的访问控制

8. **多模型支持**
   - 支持切换不同AI模型
   - 根据问题复杂度自动选择
